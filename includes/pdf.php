...
if ((isset($_GET['start'])) && (isset($_GET['end']))) {
    ...
    $result = $conn->query($query);
    // Directory for storing individual PDFs
    $pdfOutputDir = 'C:/wamp64/www/semcom_portal/assets/pdf/';
    if (!is_dir($pdfOutputDir)) {
        mkdir($pdfOutputDir, 0777, true);
    }

    // Generate individual PDFs for each enrollment number
    while ($row = $result->fetch_assoc()) {
        ...
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Your Name');
        $pdf->SetTitle('Student Profile - ' . $data['roll_no'] . '-' . $data['full_name']);
        $pdf->SetSubject('Student Profile Details');
        $pdf->SetKeywords('TCPDF, PDF, student, profile');
    ...
    $zipFileName = 'student_profiles_' . date('Ymd_hia') . '.zip';
    $zipFilePath = $pdfOutputDir . $zipFileName;

    $zip = new ZipArchive();
    if ($zip->open($zipFilePath, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
        $files = glob($pdfOutputDir . '*.pdf');
        ...
        // Clean up: delete the zip file and individual PDFs after download
        unlink($zipFilePath);
        foreach ($files as $file) {
            unlink($file);
        }
