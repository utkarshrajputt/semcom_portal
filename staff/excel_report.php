<div class="container">
    <form method="post" action="../includes/selected_fetch_excel.php" onsubmit="return validateExcelForm()">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
            // Retrieve the staff_email value from PHP
            var staff_email = "<?php echo $staff_email; ?>";

            // Add click event listener to the button
            document.getElementById('all_submit').addEventListener('click', function() {
                // Redirect to the specified URL with the staff_email parameter
                window.location.href = "../includes/all_excel.php?staff_email=" + encodeURIComponent(staff_email);
            });
        });
    ...
    // Function to toggle sections based on radio button selection
    // function toggleEntry(type, entryExcelType) {
    //     if (type === 'excel') {
    //         document.getElementById('excelSelectTable').style.display = entryExcelType === 'selectTable' ? 'block' : 'none';
    //         document.getElementById('excelSelectTableColumn').style.display = entryExcelType === 'selectTableColumn' ? 'block' : 'none';
    //     }
    // }
